...
打开浏览器访问 `http://127.0.0.1:8765`。页面只有一个入口 `/`，四个功能在同一网址内切换：运行助手、探索结构、分析质量、规划任务。每个功能都有对应默认任务、模式、执行入口和独立主工作区：运行助手是对话控制台，探索结构是代码地图，分析质量是风险矩阵，规划任务是重构看板；也支持代码库内终端探索、持久化笔记、最近笔记和会话统计。
